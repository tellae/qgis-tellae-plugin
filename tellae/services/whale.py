...
def download_from_binaries(info, handler, to_json=True):

    def tmp_handler(result):
        # fetch the binary from the download url returned by whale
        fetch_url = result["content"]["Location"]
        request(fetch_url, handler=handler, to_json=to_json)

    # call whale to get a temporary download url
    request_whale(f"/binaries/{info}/url", handler=tmp_handler)
